...
void move(chess::Board& board, BrainRot& bot) {
    if (board.isHalfMoveDraw()) {
        std::pair<chess::GameResultReason, chess::GameResult> result = board.getHalfMoveDrawType();
...
std::map<char, Texture*> loadPiecesTextures(SDL_Renderer* renderer) {
...
int main(int argc, char* argv[]) {
    ...
    std::map<char, Texture*> piecesTextures = loadPiecesTextures(renderer);
    ...
    while (!done) {
        ...
        for (std::vector<std::string>::reverse_iterator it = moves.rbegin(); it != moves.rend(); ++it)
        ...
        float displacementX = width / 2.f - std::min(width, height) / 2.f;
        float displacementY = height / 2.f - std::min(width, height) / 2.f;
